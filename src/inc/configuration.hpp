...
namespace details {
  ...
  class section_node<Section, Sections...> : public Section,
                                             public section_node<Sections...> {
  public:
    using section_t = Section;
    using base_t = section_node<Sections...>;
  ...
  public:
    inline constexpr section_node(section_t const& current, base_t const& base)
        : section_t{current}
        , base_t{base} {
    }
  };
  ...
  template<typename Used, typename Interface>
  struct ptype_inherit : std::conditional<is_ptype_satisfied_v<Used, Interface>,
                                          Interface,
                                          empty_ptype> {};
  ...
  public:
    ...
    inline constexpr built(section_t const& current, previous_t const& previous)
        : base_t{current, previous} {
    }

    template<template<typename> class This, typename Unlocked, section Appended>
    using builder_t =
  ...
  struct build_reproduction_context<Built, std::false_type> {
    using type = reproduction_context_with_scaling<typename Built::population_t,
                                                   typename Built::statistics_t,
                                                   typename Built::crossover_t,
                                                   typename Built::mutation_t,
                                                   typename Built::evaluator_t,
...
public:
  ...
  inline constexpr auto observe(gal::observe<Events, Observers>... observers) {
...
struct couple_ptype : public details::ptype_base<Built, couple_ptype> {
  ...
  template<coupling_factory<reproduction_context_t, selection_result_t> Factory>
...
public:
  ...
  using pruning_kind_t = typename pruning_traits<Population, pruning_t>::kind_t;
...
template<typename Built>
struct statistics_ptype : public details::ptype_base<Built, statistics_ptype> {
...
struct evaluate_ptype : public details::ptype_base<Built, evaluate_ptype> {
  ...
  inline constexpr auto evaluate(Evaluator const& evaluator,
    ...
    return this->next(
        evaluate_body<Evaluator, chromosome_t, Comparator>{evaluator, compare});
  }
};
...
struct builder
    : details::
          builder_node<Entries, plist<Root>, plist<>, details::empty_section> {
};
